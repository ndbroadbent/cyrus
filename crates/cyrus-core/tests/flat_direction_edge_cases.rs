...
fn test_solve_singular_system() {
    ...
    let n = vec![f64_vec(&[1.0, 1.0]), f64_vec(&[1.0, 1.0])];
    let k = i64_vec(&[1, 2]); // Inconsistent
...
fn test_compute_n_matrix_permutations() {
    ...
    let m = i64_vec(&[0, 0, 0, 1]); // Indices 0..3. 3 is the one.
...
fn test_compute_flat_direction_full() {
    ...
    assert!((res.p[0].get() - 0.5).abs() < 1e-10);
...
fn test_solve_linear_system_faer_singular() {
    ...
    if let Some(p) = res {
        ...
        assert!(p.iter().all(|x| x.get().is_finite()));
    }
}
...
fn test_solve_linear_system_faer_identity() {
    ...
    assert!((res[0].get() - 5.0).abs() < 1e-10);
    assert!((res[1].get() - 3.0).abs() < 1e-10);
}
...
fn test_compute_flat_direction_full_singular() {
    ...
    let k = i64_vec(&[1, 1]);
    let m = i64_vec(&[0, 1]); // Zeros out first column
