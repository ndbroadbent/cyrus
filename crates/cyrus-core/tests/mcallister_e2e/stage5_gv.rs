...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now tests the obvious full star-union shared-face fan as a secondary-cone certificate; the current 4-214-647 star-sample report completes in 23.42s and shows all four weighted-P2 rows are on the wall, with four shared-face simplices, six native secondary-cone hyperplanes, min/max pairing 0, six zero pairings, no negative pairings, and neither height sign strictly inside",
