...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "checkpoint-t corrected-chamber top-toric local-HKTY diagnostics can be enabled with CYRUS_CORRECTED_CHAMBER_TOP_TORIC_LOCAL_GV=1; an optimized unwind diagnostic for 4-214-647 checked the first 10 top contribution classes, including the kklt_idx=52 gv=-2 classes, and every local one-dimensional-ray HKTY value matched the toric formula value",
        ...
        remaining_gaps: vec![
            ...
            "The leading checkpoint-t corrected-chamber GV target offenders are not explained by pair-vs-four-term pruning, by the old no-gamma convention, by a uniform ±1 ambient gamma index shift, by a mismatch between corrected_heights.dat and the checkpoint-t-induced chamber, by corrected-chamber divisor chi, by dropping tiny positive q.t curves, by isolating or sign-flipping B-field parity/q.t branch buckets, by a Rust/CYTools Mori-cap selection divergence, by toric-formula provenance for the top offenders, or by the exact local-HKTY values of the first 10 top toric contributors; next diagnostics should compare aggregate per-curve inclusion/sign conventions against a direct CYTools/Python correction trace",
