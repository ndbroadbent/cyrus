...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        remaining_gaps: vec![
            ...
            "The Kähler-vector residual is now traceable further upstream to the corrected KKLT fixed-point path: corrected_target_volumes.dat is understood as corrected-chamber classical KKLT divisor volumes and matches McAllister corrected_kahler_param.dat at relative_l2=0.00010542596827921091, but Cyrus' computed corrected t still differs from that same checkpoint by relative_l2=0.02203393739805434 and max_abs=0.0977369510437136; the approximate-chamber small-curve correction is invalid at checkpoint t, so the next audit should focus on corrected-chamber GV target-correction coverage/conventions and fixed-point iteration, not divisor χ or checkpoint-file semantics",
