...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now reports local-charge signature families for accepted source-derived GV imports; the guarded 4-214-647 report records -1,-1,-1,1,1,1:28, -1,-1,-1,1,2:20, -1,-1,1,1:3, and -1,-1,-1,-1,1,3:1, with the current residual predecessors still in the -1,-1,-1,1,2 family",
            "mcallister_gv_context now serializes the local cygv skeleton and unit phase probe for residual source predecessors; the guarded 4-214-647 report shows both residual source rows use compact including-origin q=[-1,-2,1,1,1], semigroup [[1]], grading [1], unit GV -2, and expected formula value -2, while still missing local_intersection_tensor and local_chamber_certificate",
            "mcallister_gv_context now certifies that residual source family as the local O(-1)+O(-2)->P^2 bundle phase with source-derived unit tensor kappa_000=1 and positive-base chamber; the real 4-214-647 report has no residual source local missing-input counts and records ready_for_actual_cygv_call for all three residual source occurrences",
