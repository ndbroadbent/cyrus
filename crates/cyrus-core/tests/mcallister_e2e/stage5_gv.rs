...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now guards path-history lower-seed pair operations above 1024 seeds so oversized targets return explicit diagnostics instead of timing out; target 0 times out under 240 seconds at seed_count=1616 without the tighter cap, while guarded target-0 and target-2 smokes return skipped_seed_pair_limit_1024 in about four seconds, target 8 with seed_count=720 still runs the lower-seed decomposition path, and a guarded all-target batch writes all nine per-target reports in 229.70s with targets 0-5 skipped and targets 6/7/8 preserving their path-support GW balance statuses",
