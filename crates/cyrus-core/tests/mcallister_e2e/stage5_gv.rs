...
fn stage5_gv_computation_roadmap() {
    ...
    // GV checkpoint validation through the mcallister_gv binary (512), plus
    // CYTools-style complete-intersection reduction from ambient intersections
    // to CY3 triple intersections (1024).
    let completed = 1u16 | 2 | 4 | 8 | 16 | 32 | 64 | 128 | 256 | 512 | 1024;
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now aggregates the degrees of parent-only q_N offsets separately from their known-qN status, so residual reports can show whether parent-domain extra terms are low-degree known toric offsets or broader unknown history",
            "mcallister_gv_context now distinguishes disabled residual q_N-domain comparisons from requested path-history probes that stop at an element limit; a target-7 schema-4 smoke report records unavailable_exceeded_element_limit_10000__target_in_closure__no_predecessor_differences instead of collapsing the rational-cone skip into not_run",
            "mcallister_gv_context now exports and top-level aggregates predecessor candidates induced by bounded lower-seed decompositions; a target-7 schema-4 smoke report materializes the A+B+B subset splits as degree pairs 8+2, 2+8, 4+6, and 6+4 with toric/source-known status preserved for each side",
            "mcallister_gv_context now promotes unknown sides of lower-seed predecessor candidates into a unique diagnostic queue; target-7/8 schema-4 smokes record degree-4 and degree-8 composite rows, all classified as not_source_degree_bounded_ray rather than uncovered source leaves, and preserve first-generation seed-sum evidence showing degree-4 toric+toric and degree-8 toric+source decompositions",
            "mcallister_gv_context now attaches cheap max-4 bounded seed decompositions to lower-seed unknown composite rows without running another compact cygv call; target-6/7/8 schema-4 smoke reports find every queued lower-seed unknown composite as a two-term bounded seed decomposition and classify every term as known toric/source qN history, including one repeated degree-4 source seed that was invisible to the first-generation reduced-seed split",
            "mcallister_gv_context now uses --run-lower-seed-diamonds to run bounded decomposition diamond qN traces for queued lower-seed unknown composites and aggregates them as integer_nonzero_gv, integer_zero_or_absent_gv, or blocked_noninteger_gw_candidates; target-6/7/8 schema-4 smokes show some mixed source+toric degree-8 diamonds materialize GV -2 or 2 while repeated/paired known-leaf composites can collapse to zero or nonintegral GW candidates, so known leaves alone are not a promotable qN history rule",
            "mcallister_gv_context now joins lower-seed predecessor splits with bounded-diamond statuses of their unknown sides; target-6/8 schema-4 smokes expose target-level candidate pairs such as known-source plus bounded integer-nonzero, known-source plus bounded noninteger-blocked, and known-toric plus bounded zero/absent composites without manual jq stitching",
            "mcallister_gv_context now runs a two-generator candidate-pair diamond for each lower-seed predecessor split when --run-lower-seed-diamonds is enabled; target-6 schema-4 pair domains are zero/absent or HKTY-noninteger blocked, while target-8 has four integer-nonzero pair domains with GV 2 and two HKTY-error pair domains, separating unknown-side nonzero diamonds from target-computing pair domains",
            "mcallister_gv_context now aggregates candidate-pair diamond GV values, compressed errors, qN trace statuses, GW noninteger counts, and target GW coefficient statuses; a target-8 schema-4 smoke reports GV 2 for the four integer-nonzero pair domains and one noninteger GW candidate for each of the two HKTY-error pair domains",
            "mcallister_gv_context now compares candidate-pair diamond GV and target GW candidates against the independent CMS expected formula sum using exact rationals; target-8's four GV=2 pair domains all mismatch expected sum 3, and target-6's zero/absent or HKTY-error pair domains also mismatch expected sum 3, so these candidate-pair diamonds are explicitly nonpromotable",
            "mcallister_gv_context now reports the signed target GW instanton-coefficient gap required for candidate-pair diamonds to reach the independent formula sum: target 8 uses pivot 203 with component -3 and is short by -3 or -5 toward required coefficient -9, target 7 uses pivot 44 with component 2 and is short by 3 or 16 toward required coefficient 6, and target 6 uses pivot 47 with component 1 and is short by 6 on the HKTY-error branch toward required coefficient 3",
            "mcallister_gv_context now exposes the same signed target GW formula-instanton balance and aggregate balance statuses for broader path-support generator probes, including HKTY-error paths: target 8 has path-support coefficient -6, required -9, and missing -3; target 7 has coefficient 3, required 6, and missing 3 despite hkty_error; target 6 has pivot 47 but no target instanton coefficient, so it is explicitly marked path_support_gw_formula_instanton_target_coefficient_missing",
            "mcallister_gv_context now guards path-history lower-seed pair operations above a configurable --lower-seed-pair-limit, defaulting to 1024, so oversized targets return explicit diagnostics instead of timing out; guarded target-0 and target-2 smokes return skipped_seed_pair_limit_1024 in about four seconds, target 8 with seed_count=720 still runs the lower-seed decomposition path, and a guarded all-target batch writes all nine per-target reports in 229.70s with targets 0-5 skipped and targets 6/7/8 preserving their path-support GW balance statuses; after switching bounded decomposition to sparse first-pair sums, a high-budget target-0 path-history probe with --lower-seed-pair-limit 2048 now completes in 83.15s and finds a 3-term lower-seed decomposition instead of timing out; --closure-generation-limit 0 now separates decomposition triage from closure expansion, and high-budget zero-generation probes map the remaining default-skipped targets at decomposition level: target 1 completes in 45.40s with seed reduction 1616->702, target 2 in 107.10s with 2560->1124, target 3 in 191.04s with 2963->1316, target 4 in 85.22s with 2212->949, and target 5 in 210.63s with 2560->1124; targets 1-4 have 3-term decompositions with four predecessor candidates, while target 5 has a 4-term decomposition with six predecessor candidates, all under stopped_generation_limit_0; the corrected-chamber lower-seed search now checks exact two-seed hits directly and skips pair sums too high in positive grading degree to combine with another seed, preserving target-5's decomposition/predecessor sample while reducing its zero-generation runtime to 128.75s; lower-seed search stats are now serialized per target and aggregated at report level, and a target-1 zero-generation smoke reports matching per-target/top-level buckets for direct scan 1616, degree bound 17, 388643 candidate pairs, 386251 unique pair sums, 2392 duplicate pair sums, and 917893 overdegree-skipped pairs while completing in 25.09s",
            ...
            "mcallister_gv_context now reports source-derived GV import statuses for uncovered source rays; the actual-local-cygv 4-214-647 report records 20 actual local cygv imports matching CMS formulas, 21 actual local cygv imports without CMS formula checks, 32 remaining full-facet CMS formula imports, 58 rows with no integral matching CMS formula, and 115 rows with no origin-circuit witness",
            "mcallister_gv_context now reports local-charge signature families for accepted source-derived GV imports; the actual-local-cygv 4-214-647 report records -1,-1,-1,1,1,1:28, -1,-1,-1,1,2:41, -1,-1,1,1:3, and -1,-1,-1,-1,1,3:1, with the current residual predecessors still in the -1,-1,-1,1,2 family",
            "mcallister_gv_context now serializes the local cygv skeleton and unit phase probe for residual source predecessors; the guarded 4-214-647 report showed both residual source rows use compact including-origin q=[-1,-2,1,1,1], semigroup [[1]], grading [1], unit GV -2, and expected formula value -2, exposing the tensor/chamber gap that the later P2-bundle certificate closes",
            "mcallister_gv_context now certifies that residual source family as the local O(-1)+O(-2)->P^2 bundle phase with source-derived unit tensor kappa_000=1 and positive-base chamber, then imports the ready source rows through actual local cygv while requiring CMS agreement when a CMS formula is present; the real 4-214-647 report has no residual source local missing-input counts and records ready_for_actual_cygv_call for all three residual source occurrences",
            "mcallister_gv_context now aggregates lower-seed diamond GV counts and pair-expanded lower-seed diamond counts; the actual-local-cygv lower-diamond 4-214-647 report records raw lower diamonds computed=6/hkty_error=3 with GV 0 for the six computed domains, and pair-expanded lower diamonds computed=9 with GV 0 for all nine domains, so these small explicit semigroups remain nonpromotable",
            ...
            "mcallister_gv_context now aggregates star-union global-basis projection and known-qN lookup statuses; a fresh no-replay 4-214-647 report has five non-weighted-P2 skips, four weighted-P2 rows with integral target/star/sum projection to the global no-origin CYTools basis, primary target/star/target±star qN lookups still unknown_not_toric_covered, and two negative-star opposites with known nonzero source-derived GV",
            "mcallister_gv_context now reports weighted-P2 star-union chamber coverage; the fresh 4-214-647 report shows all four weighted rows are blocked because the two star simplices miss the origin-circuit exclusive target-relation points, so integral global-basis projection is not a local chamber certificate",
            "mcallister_gv_context now has an opt-in --probe-star-union-path-history diagnostic for the weighted-P2 target+star chamber row; a target-8 smoke on the current star-sample context completed in 16.55s and reports the degree-6 target+star row with 463 seeds, 355 reduced seeds, 65538 first-layer closure elements, target_in_closure=false, not_found_up_to_4 lower-seed decomposition, and top-level status stopped_generation_limit_1; with streaming element-limit enforcement, the formerly timing-out generation-2 probe now completes in 24.50s with status exceeded_element_limit_150000_during_generation_2, 150000 retained closure elements, generation-2 partial count 84462, and target_in_closure=false",
            "mcallister_gv_context now tests the obvious full star-union shared-face fan as a secondary-cone certificate; the current 4-214-647 star-sample report completes in 23.40s and shows all four weighted-P2 rows are on the wall, with four shared-face simplices, six native secondary-cone hyperplanes, min/max pairing 0, six zero pairings, no negative pairings, neither height sign strictly inside, and exact zero-pairing wall circuits serialized, including the target-7/8 resolved-conifold star-side circuit [(0,-1),(55,-1),(195,1),(212,1)]",
            "mcallister_first_principles now exports secondary_cone_heights_for_missing in corrected-chamber GV contexts, and mcallister_gv_context validates that the origin-included height vector is finite, has zero origin normalization, and matches the no-origin q-matrix width plus one, setting up direct comparison between local wall circuits and the actual corrected global chamber height vector",
            "mcallister_gv_context now evaluates the shared-face star-union wall circuits on the actual corrected global secondary height vector; a fresh 4-214-647 report from /tmp/cyrus_gv_context_global_height_report.json has height_vector_count=219, five non-weighted rows not evaluated, and all four weighted rows crossing oriented walls with six nonzero global-height pairings each (four positive, two negative), so the visible shared-face wall fan is not compatible with the corrected global chamber height",
            "mcallister_gv_context now computes the regular triangulation induced by the corrected global height on each local star-union point set; /tmp/cyrus_gv_context_global_regular_report.json shows all four weighted rows match the serialized star extras over the shared face (targets 3/6 use extras [2,46], targets 7/8 use [195,212]), while none selects the target-exclusive relation points over that face, so the serialized star chamber is the actual local global-height chamber rather than a missing target chamber",
            "mcallister_gv_context now evaluates each target origin-circuit relation against the actual corrected global secondary height vector and compares the signed pairing to the target branch q.t diagnostic; this records the wall-crossing datum for the missing target relation separately from the star-union shared-face chamber checks",
            "mcallister_gv_context now records corrected global secondary-height pairings on each star-union global-basis lookup row; the current 4-214-647 role-height report shows targets 7/8 have positive target and target±star rows, while the shared star row is negative with positive known-source-GV opposite, making the transport object explicit in the same rows as degree and known-qN status",
            "mcallister_gv_context now serializes the star-union transport decomposition target = target_plus_star + opposite(star); the current 4-214-647 transport report shows 8 projected rows satisfy this identity, and targets 7/8 split the unknown degree-10 target into an unknown positive degree-6 target_plus_star row plus a known positive degree-4 source-GV opposite-star row with height pairings adding back to q.t",
            "mcallister_gv_context now reconstructs the target_plus_star point-support affine charge basis; the current 4-214-647 support report shows targets 7/8 are two-parameter integral relations on affine-rank-4 seven-point supports, not one-parameter scalar residuals",
            "mcallister_gv_context now runs a local-cygv readiness diagnostic on the target_plus_star support; the current 4-214-647 report has all 9 target-plus-star handoffs blocked, targets 7/8 are Calabi-Yau-charge two-parameter supports with q_rows=7, q_cols=2, cy_dim=4, and recomputed single-column omissions for targets 7/8 remain Calabi-Yau-charge cy_dim=4 one-parameter supports, so they are not valid compact threefold cygv inputs",
            "mcallister_gv_context now recomputes target_plus_star two-column omissions from the remaining point coordinates; for targets 7/8 exactly four omitted pairs each become compact CY3-shaped one-parameter sub-supports, but all drop nonzero original target-relation coefficients so they remain diagnostic rather than the original degree-6 two-parameter chamber row",
            "mcallister_gv_context now checks the exclusive-pair resolved-support secondary cone for the weighted-P2 rows; the fresh 4-214-647 report has one hyperplane for each candidate but affine-height pairing 0 for all four rows, so this candidate lies on the wall rather than supplying a strict local chamber certificate",
            "A no-replay finite-semigroup pruning run is now recorded separately from the checkpoint-faithful pair rule: input-chamber 419 subcutoff curves prune to 339, checkpoint-t corrected chamber 556 subcutoff curves prune to 400 with zero toric-missing rows, and the solved-t corrected chamber 561 subcutoff curves prune to 399 with zero toric-missing rows; the final V_string is 4711.504343103075 with corrected-volume residual about 0.071843868, so finite selected-set pruning does not explain the corrected-target residual",
            "The corrected-chamber GV trace now records the active pruning strategy, generic pruned counts, and sparse per-curve KKLT target-correction vectors; a finite-semigroup trace at /tmp/cyrus_finite_semigroup_pruning_trace_with_contrib.json contains 400 pruned toric-covered curves and exposes the same tiny-volume two-face offender families as the largest target-correction L2 rows",
            "mcallister_gv_context now source-maps the weighted-P2 rank-three split-bundle obstruction: CCIT smalllinebundle/smallvb gives the product modification for O(-1)+O(-1)+O(-2), exact non-equivariant codim-2 numerator truncation is zero in integer sectors, the exact equivariant p^2 numerator starts at positive lambda order, adjacent K_P(1,1,2) p^2/q=x exp(4h) data is not reusable, and any nonzero contribution must come from equivariant residue or twisted big-J/pairing data",
            "mcallister_gv_context now top-level aggregates the weighted-P2 Chen-Ruan source-map blockers for the split-bundle route, including p^2 dual-basis readout, GW extraction, required source data, and promotion statuses, so report consumers can see the equivariant-residue/twisted-big-J gap without scanning per-generator samples",
            "mcallister_gv_context now traces the split-bundle equivariant dual-basis p^2 numerator readout from the ordinary fun_0 coefficient divided by 2 lambda, matching the CCIT small-J dual-basis convention; the first integer sectors still start at positive lambda order, so full I-function pairing or residue data remains required",
            "mcallister_gv_context now folds the weighted-base denominator constants into that dual-basis p^2 pre-mirror hypergeometric readout; for the first integer sectors the denominator constants are 2 and 96 and the readout still has positive lambda order, so mirror-map, pairing, or residue data remains the exact blocker",
            "mcallister_gv_context now expands the full weighted-base denominator through p^2 before dual-basis readout for the split-bundle integer sectors; for d=1 it records D(p)=2+10p+18p^2 and for d=2 D(p)=96+688p+2072p^2, with the fun_0/(2 lambda) readout unchanged from the denominator-constant check and still positive lambda order, so denominator p-terms do not supply a promotion rule",
            "mcallister_gv_context now records the z-order of the split-bundle full-hypergeometric dual-basis p^2 readout; the primary z^-2 coefficient vanishes for the first integer sectors while the first nonzero terms appear at z^-3, so the remaining nonzero signal is descendant/big-J/pairing data rather than a primary p2 insertion",
        ],
        remaining_gaps: vec![
            ...
            "Finite selected-set pruning is implemented and runnable, but it is not a full Hilbert-basis reduction of the ambient Mori cone and does not fix the corrected-chamber residual; a faithful sums-of-others/Hilbert-basis policy for GA production still needs either a proof that the finite selected set is complete for the chamber/cutoff or a source-derived cone reduction",
            ...
            "For the two integral weighted-P2 star-union rows, a fresh target-specific origin-witness certificate check with two LP anchors and facet-union cap 1024 still gives no supporting-face certificate: target 7 facet unions return lp_no_certificate at rank 194, target 8 facet unions return lp_no_certificate at rank 177, and relation supports remain single-generator codimension-213 skips",
            ...
            "The weighted-P2 split-bundle source route is narrowed to equivariant residue or twisted big-J/pairing data; Cyrus does not yet derive that qN/GW extraction, so the split-bundle rows remain nonpromotable",
