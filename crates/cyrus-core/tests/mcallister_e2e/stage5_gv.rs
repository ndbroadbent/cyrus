...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now source-maps the weighted-P2 rank-three split-bundle obstruction: CCIT smalllinebundle/smallvb gives the product modification for O(-1)+O(-1)+O(-2), exact non-equivariant codim-2 numerator truncation is zero in integer sectors, adjacent K_P(1,1,2) p^2/q=x exp(4h) data is not reusable, and any nonzero contribution must come from equivariant residue or twisted big-J/pairing data",
        ],
        remaining_gaps: vec![
