...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        remaining_gaps: vec![
            ...
            "The leading checkpoint-t corrected-chamber GV target offenders are not explained by pair-vs-four-term pruning, by the old no-gamma convention, by a uniform ±1 ambient gamma index shift, by toggling the ambient origin entry in gamma, by ambient-vs-basis gamma contraction, by corrected_target_volumes.dat semantics, by a mismatch between corrected_heights.dat and the checkpoint-t-induced chamber, by corrected-chamber divisor chi, by dropping tiny positive q.t curves, by continuous dropping of individual toric-covered curves, by isolating or sign-flipping B-field parity/q.t branch buckets, by isolating or sign-flipping local-formula source buckets, by real-dilog numerical evaluation for the top small-q.t contributors, by a Rust/CYTools Mori-cap selection divergence, by toric-formula provenance for the top offenders, by the exact local-HKTY values of the first 10 top toric contributors, or by any single ambient mod-2 gamma shift matching the relaxed bounded-sign LP near-bound pattern; a relaxed bounded-sign LP can nearly fit the checkpoint vector but only with many arbitrary sign flips, so the next audit should compare exact B-field/sign conventions against a direct CYTools/Python correction trace",
