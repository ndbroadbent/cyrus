...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "checkpoint-t corrected-chamber GV branch-bucket diagnostics now split the toric-covered target correction by B-field parity and q.t; for 4-214-647 the largest aggregate l2 bucket is odd-parity q.t>=0.1 with count=297 and bucket_l2=0.2334903638965792, while the two even near-flop singleton buckets at q.t≈0.004802446698931817 and q.t≈0.007120534449768456 are important top-divisor offenders but dropping either still leaves max_abs=0.10111964958304713",
            "checkpoint-t corrected-chamber GV target diagnostics now report the largest per-divisor deltas and per-curve contributions; the leading 4-214-647 offenders are dominated by tiny-volume toric classes such as [(2,1),(50,-1),(54,1),(197,-1)] with q.t≈0.004802446698931817 and no detected decomposition into up to three smaller selected candidates",
            ...
            "generic no-assertion runs default K/M flux vectors to Cyrus' computed dual basis instead of the McAllister [3,4,5,8] source basis; --dual-basis remains available for validation-source flux files",
        ...
        remaining_gaps: vec![
            ...
            "Corrected-chamber ray plus LP-witness diagnostics cover 10/10 toric-missing curves with diagnostic volume correction -0.2597648038721059, zero=10, nonzero=0, and delta_vs_toric_covered=0; the LP-witness values are not certified supporting-face computations and remain non-production diagnostics, but final-chamber missing GV values alone no longer explain the 0.072 volume residual",
            ...
            "The leading checkpoint-t corrected-chamber GV target offenders are not explained by pair-vs-three-term pruning, by a uniform ±1 ambient gamma index shift, by a mismatch between corrected_heights.dat and the checkpoint-t-induced chamber, by corrected-chamber divisor chi, by dropping tiny positive q.t curves, by isolating B-field parity/q.t branch buckets, or by a Rust/CYTools Mori-cap selection divergence; next diagnostics should compare per-curve GV values and sign conventions against a direct CYTools/Python correction trace",
