...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        remaining_gaps: vec![
            ...
            "The Kähler-vector residual is now traceable further upstream to corrected target τ construction: corrected_target_volumes.dat differs from Cyrus' computed GV-corrected KKLT target vector by relative_l2=0.022036578712025 and max_abs=0.09773695104398727, equivalent to an implied divisor-χ delta with relative_l2=0.09790035863637207 and max_abs=2.3456868250556946; Python McAllister χ(D) agrees with Cyrus on the top offenders, so the next audit should focus on GV target-correction coverage/conventions and the meaning of corrected_target_volumes.dat",
