...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now guards path-history lower-seed pair operations above 2048 seeds so oversized targets return explicit diagnostics instead of timing out; a target-2 generation-limited smoke returns skipped_seed_pair_limit_2048 with seed_count=2560 in about four seconds, while a target-8 control with seed_count=720 still runs the lower-seed decomposition path",
