...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now runs a local-cygv readiness diagnostic on the target_plus_star support; the current 4-214-647 report has all 9 target-plus-star handoffs blocked, and targets 7/8 are Calabi-Yau-charge two-parameter supports with q_rows=7, q_cols=2, cy_dim=4, so they are not valid compact threefold cygv inputs",
