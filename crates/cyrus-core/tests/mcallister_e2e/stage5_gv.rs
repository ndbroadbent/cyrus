...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        remaining_gaps: vec![
            ...
            "The leading checkpoint-t corrected-chamber GV target offenders are not explained by pair-vs-three-term pruning, by a uniform ±1 ambient gamma index shift, by a mismatch between corrected_heights.dat and the checkpoint-t-induced chamber, by corrected-chamber divisor chi, or by a Rust/CYTools Mori-cap selection divergence; next diagnostics should compare per-curve GV values and sign conventions against a direct CYTools/Python correction trace",
