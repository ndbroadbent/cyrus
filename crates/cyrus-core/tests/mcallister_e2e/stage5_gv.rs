...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context now has an opt-in --probe-star-union-path-history diagnostic for the weighted-P2 target+star chamber row; a target-8 smoke on the current star-sample context completed in 16.55s and reports the degree-6 target+star row with 463 seeds, 355 reduced seeds, 65538 first-layer closure elements, target_in_closure=false, not_found_up_to_4 lower-seed decomposition, and top-level status stopped_generation_limit_1",
