...
fn stage5_gv_computation_roadmap() {
    ...
    let roadmap = GvComputationRoadmap {
        ...
        verified_components: vec![
            ...
            "mcallister_gv_context --scan-local-integer-tensors finds tensor_value=1 matching the expected local toric formula for 4/9 missing targets, but these remain uncertified because local q-matrix phase, local intersection tensor, and chamber certificate inputs are still not source-derived",
