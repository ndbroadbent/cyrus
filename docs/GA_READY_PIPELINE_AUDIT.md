...
| Corrected-chamber face/chamber search support | `fine_regular_triangulation_choices_on_polytope_2faces_4d_with_sampling` now supplies CYTools-style exact-or-`grow2d` sampled FRT choices for every 4D two-face; sampled native/star expanded-secondary inequality builders and mixed-radix chamber materializers now mirror the exact small-face helpers; `mcallister_first_principles --dump-corrected-chamber-face-triangulation-choice-summary` exports the exact/sampled face counts, per-face choice counts, seed, and sampling thresholds without loading downstream GV data; `corrected_chamber_face_triangulation_choice_summary_tracks_sampled_faces` and the `triangulation::secondary` sampled chamber tests pin this on `P^4` simplex sampled-face cases | Implemented as chamber infrastructure; not a GV promotion |
