...
3. Reconstruct the actual corrected-chamber context for the nine remaining
   ...
   that a target curve spans an extremal Mori ray, plus
   `check_stable_weyl_candidate_certificate` to require all of these algebraic
   ...
   source-level certification step that finds the separator from exact geometry,
   proves the relevant Kähler wall/chamber transition, and identifies the
   shrinking divisor from geometry rather than from a fitted candidate list. The
