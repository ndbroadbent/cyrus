...
| Flop/corrected-chamber continuation | Negative small-curve volumes and real-axis dilog branch behavior are classified; even-parity branch-cut failures are explicit via `GvDilogFailure`; exact flop transforms for `kappa`, `c2`, GV reassignment, the stable-Weyl reflection matrix, and the Weyl-vs-flop tensor equality check are now exposed as algebraic primitives | Diagnosed, not resolved |
...
3. Reconstruct the actual corrected-chamber context for the nine remaining
   ...
   and the exact Weyl-transformed intersection tensor comparison against the
   flop-updated tensor. It still lacks the source-level certification step that
   proves a shrinking divisor exists on the dual facet for each candidate. The
