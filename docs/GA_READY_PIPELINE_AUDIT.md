...
| CYTools/cygv generic GV wrapper contract | `compute_gv_invariants` now uses CYTools-style `min_points=100*h11` lattice augmentation even when `max_deg` is supplied; bounded `max_deg` lattice enumeration is isolated in `compute_gv_invariants_with_degree_bounded_lattice`; compact GV execution still runs the upstream Rust `cygv` HKTY modules, but Cyrus now constructs the `cygv::Semigroup` and maps semigroup/intersection/fundamental-period/instanton/series-inversion failures into `Result` errors instead of relying on `cygv::compute_gv_rat_threefold` unwraps; CYTools-style matrix-basis projection helpers now match `mori_cap_matrix.dot(basis.T)`; matrix divisor-basis to dual curve-basis construction is available through `compute_curve_basis_matrix_from_divisor_basis_matrix`; `DivisorBasis` and `compute_curve_basis_matrix_for_divisor_basis` make vector-vs-matrix curve-basis dispatch explicit; `project_mori_cone_cap_rays_for_divisor_basis` and `gv_divisor_basis_data` bundle Mori projection, curve-basis construction, and no-origin q-matrix construction for either basis shape; `intersection_in_divisor_basis` now dispatches vector filtering and matrix-basis dense tensor pullback for the in-basis intersection tensor passed to `cygv`; `mcallister_first_principles`, `mcallister_gv`, and `mcallister_racetrack` vector-basis GV paths now use that bundled handoff instead of independently projecting Mori rays and hand-building q matrices; `curve_basis_matrix_without_origin_i64` and `curve_basis_q_matrix_for_divisor_basis_i64` centralize the `curve_basis(include_origin=False, as_matrix=True)` q-matrix boundary used by cygv call sites; `mcallister_first_principles --dual-basis` now parses matrix-basis JSON but rejects it with an explicit unsupported-vector-path error instead of silently treating it as indices | Core GV input construction now supports vector or matrix bases for Mori rays, q matrices, curve-basis matrices, and in-basis intersections. McAllister runner remains vector-basis only and must keep rejecting matrix overrides until flux, Kähler, and branch-volume transforms are generalized |
...
2. Close the remaining CYTools basis contract gap for GA use: matrix-basis
   ...
   Higher-level APIs still need to either accept a generic matrix basis
   end-to-end or reject it loudly until flux, Kähler, and branch-volume
   transforms are ported.
