...
| Corrected-chamber face/chamber search support | `fine_regular_triangulation_choices_on_polytope_2faces_4d_with_sampling` now supplies CYTools-style exact-or-`grow2d` sampled FRT choices for every 4D two-face; sampled native/star expanded-secondary inequality builders and mixed-radix chamber materializers now mirror the exact small-face helpers; `expanded_secondary_face_choice_indices_containing_height_vector` selects which per-face blocks a height vector strictly lies inside; `mcallister_first_principles --dump-corrected-chamber-face-triangulation-choice-summary` exports exact/sampled face counts, per-face choice counts, height-compatible choice indices/counts, unique mixed-radix chamber digits/index when available, seed, and sampling thresholds without loading downstream GV data; when the solved height vector selects a unique per-face choice, the same summary now materializes the selected expanded-secondary chamber hyperplanes and records their height-pairing certificate; `--dump-corrected-chamber-gv-context` can embed the same expensive summary only when `--include-corrected-chamber-face-triangulation-choice-summary-in-gv-context` is set, and `mcallister_gv_context` validates the summary's status, min/max counts, total mixed-radix product, sampled-face threshold, compatible-choice counts, unique digit/index consistency, and selected-chamber certificate before reporting top-level face-choice and selected-chamber fields; `corrected_chamber_face_triangulation_choice_summary_tracks_sampled_faces`, `expanded_secondary_face_choice_selection_uses_height_pairings`, `validate_context_reports_face_triangulation_choice_summary`, and the sampled chamber tests pin this on square and `P^4` simplex cases | Implemented as chamber infrastructure; not a GV promotion |
