...
| Potent-ray convergence data | `curve_row_span_rank`, `potent_ray_convergence`, `diagnose_affine_toric_circuit`, `compute_ray_gv_series_with_provided_generators`, `compute_one_dimensional_ray_gv_series`, and `stage5_mcallister_potent_ray_checkpoint_quantities_are_computed` compute rank, corrected-Kähler volumes, affine local-circuit structure, `log xi_n` slopes, and expose reusable cygv ray-series entry points | Partially implemented; ray sampling and 4-214-647 face-context ray-series regeneration still missing |
