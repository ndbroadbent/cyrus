...
2. Close the remaining CYTools basis contract gap for GA use: matrix-basis
   ...
   initialization-scaling APIs now accept matrix Kähler-coordinate bases. The
   first-principles runner still needs to wire those through branch-volume and
   chamber-diagnostic paths, or reject matrix production bases loudly at those
   stage boundaries.
