...
3. Reconstruct the actual corrected-chamber context for the nine remaining
   ...
   quadratic volume form vanishes identically on the hyperplane `C.t=0`. These
   are necessary algebraic pieces for the stable-Weyl test, but they are not a
   chamber certificate by themselves. Cyrus still needs the source-level
   certification step that proves the curve is a supporting Mori-facet
   generator and identifies the shrinking divisor from geometry rather than
