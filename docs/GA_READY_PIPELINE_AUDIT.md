...
| Corrected-chamber face/chamber search support | `fine_regular_triangulation_choices_on_polytope_2faces_4d_with_sampling` now supplies CYTools-style exact-or-`grow2d` sampled FRT choices for every 4D two-face; sampled native/star expanded-secondary inequality builders and mixed-radix chamber materializers now mirror the exact small-face helpers; `expanded_secondary_face_choice_indices_containing_height_vector` selects which per-face blocks a height vector strictly lies inside; `mcallister_first_principles --dump-corrected-chamber-face-triangulation-choice-summary` exports exact/sampled face counts, per-face choice counts, height-compatible choice indices/counts, unique mixed-radix chamber digits/index when available, seed, and sampling thresholds without loading downstream GV data; when the solved height vector selects a unique per-face choice, the same summary now materializes the selected expanded-secondary chamber hyperplanes and records their height-pairing certificate; routine `--dump-corrected-chamber-gv-context` exports now include a cheap `induced_face_triangulation_chamber_certificate` computed directly from the solved triangulation, with two-face counts, induced triangle counts, and a two-face secondary height certificate; the expensive full face-choice summary is embedded only when `--include-corrected-chamber-face-triangulation-choice-summary-in-gv-context` is set, and `mcallister_gv_context` validates both certificate families before reporting induced-face chamber fields, top-level face-choice fields, and selected-chamber fields; `induced_face_triangulation_chamber_certificate_tracks_face_triangles`, `validate_context_reports_induced_face_chamber_certificate`, `corrected_chamber_face_triangulation_choice_summary_tracks_sampled_faces`, `expanded_secondary_face_choice_selection_uses_height_pairings`, `validate_context_reports_face_triangulation_choice_summary`, and the sampled chamber tests pin this on square and `P^4` simplex cases | Implemented as chamber infrastructure; not a GV promotion |
...
4. Implement explicit chamber/flop continuation rules for the Kähler-coordinate
   ...
   The corrected all-target aggregate reports
   `positive_degree_current_chamber_only_contains_target_plus_star:3`,
   ...
   The codimension-two nef-partition scan now certifies every small partition
   candidate with the CYTools-style support-polytope check. The refreshed
   `/tmp/cyrus_nef_candidate_cert_all_v1.json` report shows zero certified
   ...
   `no_compact_threefold_omission_candidates`. Thus the obvious local support
   enlargement route remains diagnostic rather than a source-certified
   ...
   `/tmp/cyrus_star_union_all_chamber_generator_local_toric_kind.json`
   ...
   `{2:2, 3:6, 4:2}`, and support sizes `{4:2, 5:6, 6:2}`. The rank-two rows
   are now shape-classified separately as `rank_two_quadrilateral` rather than
   left as unidentified (`current:5`, `flipped:2`), but this classification is
   deliberately not a GV formula. Existing CKYZ source matching remains
   negative for these chamber rows: current CKYZ statuses are
   `ckyz_no_source_match:5` and `ckyz_not_run_non_rank_two_support:10`, while
   flipped statuses are `ckyz_no_source_match:2` and
   `ckyz_not_run_non_rank_two_support:8`. The actual decomposition terms for
   targets `1`, `7`, and `8` inherit that same conclusion: the rank-two terms
   are recognized quadrilateral/conifold-like local circuits but still lack a
