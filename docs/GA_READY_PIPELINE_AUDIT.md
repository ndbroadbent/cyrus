...
| Mirror-side racetrack GV data | `mcallister_gv` computes compact mirror-side GV invariants with the actual Rust `cygv` min-points handoff, maps basis curves back to ambient classes, and validates every `dual_curves.dat` / `dual_curves_gv.dat` checkpoint row | Implemented for 4-214-647: `--min-points 20000` produces 10556 ambient invariants and matches all 5177 checkpoint rows |
