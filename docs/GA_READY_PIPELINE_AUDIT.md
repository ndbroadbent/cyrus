...
3. Reconstruct the actual corrected-chamber context for the nine remaining
   ...
   exact algebraic flop transforms and the exact Weyl reflection matrix
   primitive, but still lacks the source-level certification step that proves a
   shrinking divisor exists on the dual facet and that the Weyl-transformed
   intersection tensor matches the flop-updated tensor. The LP-witness face
   diagnostic is useful but currently uncertified, so it must remain a
