...
targeted first-principles F1 direction `[2,1]` now has an explicit all-ten
regression against the saved row and the existing target-downset all-row gate
remains the practical default. Flattening the target-downset addition table
...
This is still not the full McAllister potent-ray solution. The F1 `[2,1]`
all-ten check proves the CKYZ source-derived path can reach a complete saved row
for at least one non-`P^2` family, but the missing step is still to derive the
local generator set and grading for each normalized support signature from
geometry, then use a narrower coefficient/history domain to raise the larger
rank-two CKYZ families beyond the first three multiples.
