...
flattened addition table also makes the narrowed polygon-5 `[4,3,2]` `N=4`
gate complete in about 178 seconds. Higher all-row/all-ten validation remains a
finite-domain construction problem rather than a known coefficient mismatch.
...
targeted first-principles F1 direction `[2,1]` passed quickly and the existing
target-downset all-row gate remains the practical default. Flattening the
target-downset addition table removes enough lookup overhead for the narrowed
polygon-5 `[4,3,2]` `N=4` check to complete, but the sample still spends almost
all time in inverse mirror-map composition and rational polynomial
multiplication. This confirms the next missing object more sharply: Cyrus needs
the coefficient/path-history domain described above, not just the full semigroup
generated by the CKYZ coordinate axes up to a source grading cutoff.
