...
targeted first-principles F0 direction `[1,1]` and F1 direction `[2,1]` now
have explicit all-ten regressions against saved rows and the existing
target-downset all-row gate remains the practical default. Flattening the target-downset addition table
...
mirror-map composition and rational polynomial multiplication. This confirms
the next missing object more sharply: Cyrus needs the coefficient/path-history
domain described above, not just the full semigroup generated by the CKYZ
coordinate axes up to a source grading cutoff.

This is still not the full McAllister potent-ray solution. The F0 `[1,1]` and
F1 `[2,1]` all-ten checks prove the CKYZ source-derived path can reach complete
saved rows for multiple non-`P^2` families, but the missing step is still to
derive the local generator set and grading for each normalized support signature
from geometry, then use a narrower coefficient/history domain to raise the
larger rank-two CKYZ families beyond the first three multiples.
