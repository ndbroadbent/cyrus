...
source qN history. Cyrus also profiles the first CCIT hypergeometric factor
ranges for bundle degrees `[1,1,2]`: half-degree terms are twisted-sector
terms, while integer untwisted terms have numerator zero order `3`, which is
larger than the codimension-`2` insertion and blocks a naive non-equivariant
`H^2` coefficient readout. The core regression
...
compatible choice, sampling limits, and seed. When those unique digits exist,
the artifact also materializes the selected expanded-secondary chamber
hyperplanes and records the corrected height vector's pairing certificate for
...
A fresh no-replay finite-semigroup pruning run applies the stricter selected-set
rule to the full runner:
...
It completes in `real 115.77`. The input chamber prunes `419 -> 339`, the
checkpoint-t corrected chamber prunes `556 -> 400` with `toric_missing=0`, and
the solved-t corrected chamber prunes `561 -> 399` with `toric_missing=0`.
The final summary is `V_string=4711.504343103075` and
`log10(|V0|)=-202.26279585143402`, leaving a corrected-volume residual of
...
1. Treat pruning policy as explicit but not as the main residual source. The
   default `pair` reproduces the 4-214 checkpoint, while
   `finite-semigroup` is runnable for GA diagnostics and removes additional
   selected-set sums without fixing the corrected volume.
2. If the
...
The default pair-pruned corrected-chamber solved-t run reports
`ambient_rays=561596`, `subcutoff=561`, `pair_pruned=419`,
`toric_covered=410`, and `toric_missing=9`. The finite-semigroup run removes
those visible toric-missing rows but leaves the same corrected-target problem,
so the pair-pruned rows remain a useful diagnostic queue rather than the whole
explanation. Each missing target is an origin-circuit Mori generator; the
...
are not reconstructed by a separate formal series. For both degree-ten targets
the path-support domain has three such lower sources: the toric degree-two
predecessor, the shared source-derived degree-six row, and the unknown
degree-eight residual. In the fresh target-`7` report their qN target-term
coefficients are `-2`, `-1`, and `1`, their exact Li2 target coefficients are
`-5/2`, `-1`, and `1`, and the pivot-coordinate subtraction coefficients are
`5`, `0`, and `-2`. Target `8` has the same qN/Li2 coefficient pattern, with
pivot-coordinate subtraction coefficients `-10`, `2`, and `2`. This is exactly
...
`/tmp/cyrus_gv_context_star_union_rational_report.json` counts are
...
`3`/`6` already fail integrality in this union basis with exact target
...
The follow-up `/tmp/cyrus_gv_context_star_union_global_basis_report.json`
uses the CYTools no-origin column convention from
`curve_basis(include_origin=False, as_matrix=True)`: the origin is dropped and
point index `p>0` maps to column `p-1`. With that map, all four weighted
union target/star rows and their target-minus-star/target-plus-star sums are
integral global curve-basis classes. The projected target rows match the
reported missing-target basis classes exactly, e.g. target `7` projects to
`[(44,2),(203,1),(206,-3),(209,1)]` and target `8` to
`[(203,-3),(206,1),(209,1)]`; the shared star row for targets `7`/`8`
...
The corrected all-target aggregate at
`/tmp/cyrus_positive_degree_transport_all_v2.json` generalizes this queue:
three rows report
`positive_degree_current_chamber_only_contains_target_plus_star`, two rows
separate out as `positive_degree_transport_zero_target_plus_star`, and four
rows do not run the positive-degree chamber check because the star-union
relation is not available integrally. The three current-only rows are exactly
...
GV context exports include `secondary_cone_2face_height_certificate`. The
context reader validates and reports this certificate explicitly, so downstream
target-plus-star diagnostics can distinguish full secondary-cone membership
from two-face-skeleton membership. On the 4-214-647 corrected-Kähler checkpoint
chamber, the two-face skeleton has `589` native hyperplanes and the corrected
...
They also aggregate the generator context itself: current chamber degrees
`{2:2, 4:2, 6:1}` with bounded lower-seed statuses
`found_lower_seed_decomposition:2` and `not_found_up_to_4:3`; flipped chamber
degrees `{-4:1, 2:1, 4:1, 6:1, 8:1}` with bounded lower-seed statuses
`found_lower_seed_decomposition:1`, `not_found_up_to_4:2`, and
`not_run_or_not_applicable:2`. Both chambers still split as
...
`/tmp/cyrus_star_union_all_chamber_generator_local_toric_kind.json`, all
...
ranks are `{2:2, 3:6, 4:2}` with support sizes `{4:2, 5:6, 6:2}`. The rank-two
rows now classify as `rank_two_quadrilateral` (`current:5`, `flipped:2`),
which records the conifold-like four-point circuit shape without assigning a
GV sequence. Existing CKYZ source matching still does not cover them: current
CKYZ statuses are `ckyz_no_source_match:5` and
`ckyz_not_run_non_rank_two_support:10`, while flipped statuses are
`ckyz_no_source_match:2` and `ckyz_not_run_non_rank_two_support:8`. The target
`1`, `7`, and `8` decomposition terms therefore remain
source-history/chamber-construction problems: the rank-two terms are recognized
quadrilateral circuits but still lack source-certified compact qN history, and
...
star-union points back into the six-point chamber-generator support. The
regenerated reports
`/tmp/cyrus_six_point_zero_enlargement_reflexivity_target7_v1.json` and
`/tmp/cyrus_six_point_zero_enlargement_reflexivity_target8_v1.json` show three
source-derived enlargements per target: target `7` adds point `46`, point
`195`, or both, while target `8` adds point `2`, point `195`, or both. All
still have hypersurface `cy_dim=4`, zero zero-degree nef candidates, and
`support_polytope_not_reflexive_origin_is_hull_vertex`. The one-point
enlargements do expose compact threefold omissions (`2` for the first added
point, `4` for point `195`), but every such omission deletes terms from the
...
The v2 target `7`/`8` artifacts also expose the source invariants:
base anticanonical degree `4`, bundle degree sum `4`, total first-Chern
degree `0`, and base hyperplane square `1/2`. Thus the visible total space is
