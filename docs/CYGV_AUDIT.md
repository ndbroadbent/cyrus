...
polygon-5 `[4,3,2]`, `N=7` regression pass in about 46 seconds; `N=8` and
`N=9` also pass as diagnostics in about 118 seconds and 274 seconds,
respectively. Focused polygon-5 `[4,3,2]`, `N=10` still exceeds a 300 second
timeout, so higher all-row/all-ten validation remains a finite source/history
construction problem rather than a known coefficient mismatch. A traced `N=10`
run sharpened this: support prediction finished quickly (`broad=26691`,
...
therefore the live residual dependency graph, not another broad support-domain
optimization.
...
in about 46 seconds. The focused `[4,3,2]`, `N=8` and `N=9` runs also pass as
diagnostics in about 118 seconds and 274 seconds, while `N=10` still times out
after 300 seconds. A fresh trace shows the support-predicted domain itself is
...
object more sharply: Cyrus needs a live residual/source-history domain for the
full predecessor subtraction path, not just the full semigroup generated by the
