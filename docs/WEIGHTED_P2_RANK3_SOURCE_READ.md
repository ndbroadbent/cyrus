...
The compact current-chamber uncertified-generator sample now carries the same
source-certificate facts without requiring callers to unpack the full local
toric diagnostic. For the blocking weighted-`P2` generator it records the zero
primary `z^-2` readout, the descendant/equivariant first-nonzero blocker, the
missing `z^-1` primary extraction status, and the checked degree/sector counts
next to the promotion status and required source inputs. This is still
bookkeeping for the obstruction, not a numerical GV promotion.
