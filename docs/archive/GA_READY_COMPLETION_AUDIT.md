...
1. **Corrected-chamber GV target corrections.** Nine current solved-t misses
   ...
   `[195,212]` are omission hits and `[0,55]` are not. The shared GKZ
   circuit-side primitive now serializes the selected facets
   `[[0,55,195],[0,55,212]]` and opposite facets
   `[[0,195,212],[55,195,212]]`, so the remaining wall-crossing work has an
   explicit chamber-facet input rather than a raw simplex-search hint. The
   compact-omission
   ...
   CKYZ matching. A full small codimension-two nef-partition certificate scan
   now also reports zero certified target-plus-star or origin-spectator CICY
   candidates; even the six zero-degree candidates on targets `7`/`8` fail the
   CYTools-style support-hull certificate with the origin missing as an ambient
   vertex. The follow-up origin-included target `7`/`8` scan shows `31`
   bipartitions and `6` zero-degree balanced candidates when point `0` is
   allowed into the parts, but all candidates fail with
...
4. **Compact GV semigroup/face certification.** Missing corrected-chamber GV
   ...
   history. The context export also profiles the first CCIT hypergeometric
   factor ranges for bundle degrees `[1,1,2]`: half-degree terms require
   twisted-sector pairing, and integer untwisted terms have numerator zero
   order `3`, exceeding the codimension-`2` insertion. A core regression now
   verifies that fractional intersection tensors are rejected before compact
   `cygv` handoff, matching the upstream HKTY `i32` intersection-number API.
...
The origin-included target `7`/`8` reports make the adjacent negative result
